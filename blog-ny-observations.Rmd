...
annotation_df <- data.frame(x = c(rep(x = as.Date("2016-04-10"), 3), as.Date("2016-03-08")),
                            y = c(31, 24, 17, 85),
                            ...
                            hjust = c(0, 0, 0, 0.5),
                            color = c(rep("#6a3d9a", 3), "grey30"))

segment_df <- data.frame(x = rep(as.Date("2016-04-08"), 3),
                       y = c(31, 24, 17),
                       xend = c(as.Date("2016-01-27"), as.Date("2016-02-06"), as.Date("2016-02-14")))
...
ggplot() + geom_line(data = temp_df, aes(x = date, y = tavg, color = location), size = 1) +
  ...
  annotate("text", x = annotation_df$x, y = annotation_df$y, size = 3, color = annotation_df$color, label = annotation_df$label, hjust = annotation_df$hjust) +
  geom_segment(data = segment_df, aes(x = x, y = y, xend = xend, yend = y), color = "#6a3d9a") +
  geom_curve(aes(x = as.Date("2016-02-15"), y = 81, xend = as.Date("2016-01-15"), yend = 33), curvature = 0.1,
             arrow = arrow(length = unit(0.02, "npc")), color = "grey30")
